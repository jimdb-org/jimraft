...
std::vector<uint64_t> random_uint_vector_repeate(uint64_t max_value,
                                                 uint64_t count)
{
    std::vector<uint64_t> rs;

    while (count--) {
        rs.push_back(rand_r(&seed) % max_value);
    }

    return rs;
}

std::vector<uint64_t> random_uint_vector_unique(uint64_t max_value,
                                                uint64_t count)
{
    std::vector<uint64_t> rs;

    while (rs.size() != count) {
        uint64_t rand_value = rand_r(&seed) % max_value;

        if (std::find(rs.begin(), rs.end(), rand_value) == rs.end()) {
            rs.push_back(rand_value);
        }
    }
...
std::string EncodeToOct(const std::string& src)
{
    std::string result;
    ...
    for (std::string::size_type i = 0; i < src.size(); ++i) {
        if (!isprint(src[i])) {
            snprintf(buf, 5, "\\%03o", static_cast<unsigned char>(src[i]));
            result.append(buf, 4);
        } else {
...
static uint64_t approximateInt(const std::string& str, size_t offset)
{
    uint64_t val = 0;
    for (int i = 0; i < 8; i++) {
        auto ch = offset < str.size() ? static_cast<uint8_t>(str[offset])

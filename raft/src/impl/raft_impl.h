...
public:
    ...
    void Propose(std::string& entry_data, uint32_t entry_flags, void *tag) override;
...
private:
    ...
    void proposeErr(std::string data, void *tag, Status s);
    void handlePropose();
    void resetPendings(const Status& s);

    void smApply(const EntryPtr& e, void *tag);
...
private:
    struct ProposeTask {
        ...
        explicit ProposeTask(int batch_size);

        void AddEntry(std::string& entry_data, uint32_t entry_flags, void *tag);
    };
    using ProposeQueue = std::deque<std::unique_ptr<ProposeTask>>;
    ...
    ProposeQueue propose_que_;
    std::mutex propose_lock_;
    std::map<uint64_t, void*> pendings_;

...
RaftServerImpl::RaftServerImpl(const RaftServerOptions& ops) :
    ops_(ops),
    tick_msg_(new pb::Message) {
...
Status RaftServerImpl::Start() {
    ...
    if (!ops_.apply_in_place) {
        for (int i = 0; i < ops_.apply_threads_num; ++i) {
            auto t = new WorkThread(ops_.apply_queue_capacity,
                                    std::string("raft-apply:") + std::to_string(i));
            apply_threads_.push_back(t);
        }
        FLOG_INFO("raft[server] %d apply threads start. queue capacity=%d",
...
Status RaftServerImpl::CreateRaft(const RaftOptions& ops, std::shared_ptr<Raft>* raft) {
    ...
    uint64_t counter = 0;
    {
        std::unique_lock<jim::shared_mutex> lock(rafts_mu_);
...
std::shared_ptr<RaftImpl> RaftServerImpl::findRaft(uint64_t id) const {
    jim::shared_lock<jim::shared_mutex> lock(rafts_mu_);
...
void RaftServerImpl::tickRoutine() {
    while (running_) {
        ...
        RaftMapType rafts;
        {
            std::lock_guard<jim::shared_mutex> lock(rafts_mu_);
            rafts = all_rafts_;
        }
        sendHeartbeat(rafts);
        stepTick(rafts);
...
void RaftServerImpl::printMetrics() {
    ...
    if (now > last && now - last > 10) {
        ...
        if (!ops_.apply_in_place) {
            ...
            apply_metrics += "]";
            FLOG_INFO("raft[metric] apply queue size: %s", apply_metrics.c_str());
        }
    }
}

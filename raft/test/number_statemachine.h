...
public:
    ...
    Status Apply(const CmdResult& cmd) override {
        if (!cmd.replicate_status.ok()) {
            throw std::runtime_error("replicate failed: " + cmd.replicate_status.ToString());
        }

        auto num = std::stoull(cmd.data);
        FLOG_INFO("node_id: {}, apply: {}, index: {},num: {}", node_id_, cmd.data, cmd.index, num);
        {
